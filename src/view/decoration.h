...
namespace umbriel {
  ...
  public:
    ...
    // Copy the border into a close-animation snapshot tree. `innerColor` and `outerColor` are the straight colours the
    // ring currently shows and `opacity` the effective opacity it is drawn at.
    void snapshotBorders(
        wlr_scene_tree* snapshot, const std::array<float, 4>& innerColor, const std::array<float, 4>& outerColor,
        float opacity, std::vector<BorderSnapshot>& out
    ) const;
